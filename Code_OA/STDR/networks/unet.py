...
class Decoder_MH(nn.Module):
    def __init__(self, params):
        super(Decoder_MH, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
    ...
    def forward(self, feature):
        ...
        x = self.up3(x, x1)
        x = self.up4(x, x0)
        output1 = self.out_conv1(x)
        output2 = self.out_conv2(x)
        output3 = self.out_conv3(x)
...
class UNet_MH(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_MH(params)
...
class UNet_CCT(nn.Module):
    ...
    def forward(self, x):
        ...
        return (aux_seg1, aux_seg2, aux_seg3)
